...
## Combined-branch integration notes
PrivacyCopyField now hides its source label synchronously when PrivacyEngine emits active_changed(active), preventing the mask's fade from revealing its value. Copy still uses the underlying string. Both demos use opaque tint and a narrow margin around critical invite fields.

The combined demos use a dismissible shared settings menu instead of the earlier dedicated privacy showcase controls. The full advanced controls remain available as addon components.

The 300-node churn benchmark exceeded its 16 ms target in integration testing (about 34 ms). The default suite reports timing and tests correctness; add -- --strict-performance to enforce the timing threshold. Performance acceptance remains open.
