...
func _test_masked_but_copy_returns_real_value() -> void:
	...
	_check(not field._value_label.visible, "Source text is hidden synchronously before mask animation")
	field.value = field.value + "9"
	_check(not field._value_label.visible, "Changing a protected value never reveals it")
	field.value = field.value.left(-1)
	controller.set_enabled(false)
	_check(field._value_label.visible, "Disabling protection restores source text")
	controller.set_enabled(true)
	await process_frame
	await process_frame
	await process_frame
